...
**加锁** setnx(SET lock_key random_value NX PX 5000)，实际是在redis中给key设置一个唯一的字符串，并设置过期时间

**解锁** LUA脚本，判断当前锁的字符串是否与传入的值相等，是的话就删除Key，解锁成功

适用于单节点Redis分布式锁，不具有可重入性

#### Redisson

**加锁** LUA脚本，使用hash结构，流程如下：
...
**加锁** 当且仅当从大多数（N/2+1）的Redis节点都取到锁，并且使用的时间小于锁失效时间时，锁才算获取成功。
